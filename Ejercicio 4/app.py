...
class Calculadora(object):
    ...
    def __init__(self):
        ...
        ttk.Button(mainframe, text='/', command=partial(self.ponerNUMERO, '/')).grid(column=4, row=6, sticky=W)
        ttk.Button(mainframe, text='AC', command=(self.borrarPanel)).grid(column=4, row=7, sticky=W)
        self.__panel.set('')
        panelEntry.focus()
    ...
    def resolverOperacion(self, operando1, operacion, operando2):
        resultado=0
        if "/" in operando1:
            numerador, denominador = operando1.split("/")
            operando1 = Fraccion(numerador,denominador)
        if "/" in operando2:
            numerador, denominador = operando2.split("/")
            operando2 = Fraccion(numerador,denominador)
        if operacion=='+':
        ...
        if isinstance(resultado,Fraccion): 
            self.__panel.set(resultado.simplifica())
        else:
